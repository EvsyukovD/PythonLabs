...
def count(f, a, b, c, d, xk, s):
    return (a + f(b, c, d) + xk) << s


def count_inv(f, y, b, c, d, xk, s):
    return (y >> s) - f(b, c, d) - xk


def md4_round(r, f, x, s, k):
    for j in range(16):
        r[(3 * j) % 4] = count(f, r[(3 * j) % 4], r[(1 + 3 * j) % 4], r[(2 + 3 * j) % 4], r[(3 + 3 * j) % 4], x[k(j)],
                               s[j % 4])
    return r
...
def md4_hash_block(s, r_prev=None):
    assert len(s) == 64
    x = list(map(Mod32, (struct.unpack('<16I', s))))

    if not r_prev: r_prev = [A, B, C, D]
    x1 = [Mod32(0x5A827999) + Mod32(x[i]) for i in range(16)]
    x2 = [Mod32(0x6ED9EBA1) + Mod32(x[i]) for i in range(16)]
...
def md4(s):
    s = addbits(s)
    arr = [s[i:i + 64] for i in range(0, len(s), 64)]
    r = [A, B, C, D]
    for j in arr:
